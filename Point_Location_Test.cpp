...
//  BIT Macros
#define isBitSet(x, i)        (((x) >> (i)) & 1) // Check if ith bit is set
#define setBit(x, i)          ((x) | (1LL << (i))) // Set ith bit
#define clearBit(x, i)        ((x) & ~(1LL << (i))) // Clear ith bit
#define toggleBit(x, i)       ((x) ^ (1LL << (i))) // Toggle ith bit
#define countBits(x)          (__builtin_popcountll(x)) // Count set bits
#define isPowerOfTwo(x)       ((x) && !((x) & ((x) - 1))) // Check power of two
#define lowBit(x)             ((x) & (-(x))) // Lowest set bit value
#define removeLowBit(x)       ((x) & ((x) - 1)) // Remove lowest set bit
#define lowBitIndex(x)        (__builtin_ctzll(x)) // Index of lowest set bit (0-based)
#define highBitIndex(x)       (63 - __builtin_clzll(x)) // Index of highest set bit (0-based)
#define binToInt(s)          stoi(s, nullptr, 2) // Binary(string) to int
//#define binToLL(s)           stoll(s, nullptr, 2) // Binary(string) 
#define intToBin(x)          bitset<32>(x).to_string() //int to binary (string)
#define llToBin(x)           bitset<64>(x).to_string()
...
void boom(){
  ...
  int cross =  1LL*(c - a)*(f-b)-1LL*(d-b)*(e-a);
  if(cross == 0){
    cout<<"TOUCH"<<endl;
  }else if(cross>0){
    cout<<"LEFT"<<endl;
