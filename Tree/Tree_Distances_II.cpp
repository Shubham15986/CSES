...
int maxi;
vi ans;
void dfs(vvi &adj, int node,int parent,int dist, vi &dp, vi &ans){
    dp[node]=1;
    ans[1]+= dist;
   for(auto it: adj[node]){
    if(it != parent){
        dfs(adj,it,node,dist+1,dp,ans);
        dp[node]+= dp[it];
    }
   }
}
void dfs2(vvi &adj, int node,int parent,int dist, vi &dp,vi &ans){
    int n = adj.size()-1;
   for(auto it: adj[node]){

    if(it != parent){
         ans[it] = ans[node] - 2*dp[it] + n;
         dfs2(adj,it,node,dist+1,dp,ans);
    }
   }
}
...
void boom(){
    ...
    dfs(adj,1,0,0,dp,ans);
    dfs2(adj,1,0,0,dp,ans);
