...
void lazy_op(int id,int L,int R,int l,int r,int val){
    if(lazy[id]!=0){
        seg[id]+= lazy[id]*(R-L+1);
        if(L!=R){
            lazy[2*id+1] += lazy[id];
    ...
    int mid = (R+L)/2;
    lazy_op(2*id+1,L,mid,l,r,val);
    lazy_op(2*id+2,mid+1,R,l,r,val);
...
void boom(){
    ...
    while(q--){
        ...
        if(o&1){
            ...
            lazy_op(0,0,n-1,l,r,val);
        }else{
            int k;cin>>k;k--;
            int ans = op_st(v,0,n-1,k,k,0);
            cout<<ans<<endl;
